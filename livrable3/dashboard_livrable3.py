...
# Dans dashboard_livrable3.py, après la section "🔬 Analyse Détaillée"
# Ajoutez cette nouvelle vue

elif view_mode == "🛡️ Tests de Robustesse":
    
    st.markdown("## Tests de Robustesse du Shield")
    st.markdown("Validation des comportements en cas d'anomalies")
    st.markdown("---")
    
    # Test 1: Valeurs aberrantes
    st.markdown("### Test 1: Valeurs aberrantes de capteurs")
    ...
    with col_t1:
        st.markdown("""
        <div class="info-box">
        <b>🔴 NaN (valeur manquante)</b><br>
        Observation: [nan, 0.5, 0.2, ...]<br>
        Détection: ✅<br>
        Correction: Valeur par défaut (0.35)<br>
        Action: Continue avec état corrigé
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_t2:
        st.markdown("""
        <div class="info-box">
        <b>⚠️ Valeur hors échelle</b><br>
        Observation: [2.5, 0.5, ...]<br>
        Détection: ✅<br>
        Correction: Clip à [0,1]<br>
        Action: Continue avec état corrigé
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_t3:
        st.markdown("""
        <div class="info-box">
        <b>🔥 Température impossible</b><br>
        Observation: T > 1200°C<br>
        Détection: ✅<br>
        Correction: Valeur par défaut<br>
        Action: STOP préventif en mode réel
        </div>
        """, unsafe_allow_html=True)
    
    # Test 2: Montée rapide
    st.markdown("### Test 2: Montée rapide de température")
    
    import plotly.graph_objects as go
    
    temps = [300, 555, 810, 1065]
    actions = ["increase_speed", "increase_speed", "reduce_speed", "emergency_stop"]
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f"{a}" for a in actions],
        textposition='top center'
    ))
    fig.add_hline(y=800, line_dash="dash", line_color="orange", annotation_text="Seuil alerte (800°C)")
    fig.add_hline(y=850, line_dash="dash", line_color="red", annotation_text="Seuil critique (850°C)")
    fig.update_layout(
        title="Réaction du shield à une montée rapide de température",
        xaxis_title="Step",
    ...
    st.markdown("""
    **Analyse:** Dès que la température dépasse 800°C, le shield interdit l'augmentation de vitesse.
    Au-delà de 850°C, le shield force l'arrêt d'urgence.
    """)
    
    # Test 3: Conflit de règles
    st.markdown("### Test 3: Conflit entre règles")
    
    conflict_df = pd.DataFrame([
        {"Règle 1": "Température élevée (>800°C)", "Règle 2": "Pression élevée (>9.0 bar)", "Conflit": "OUI", "Résolution": "Priorité à la plus restrictive", "Action": "reduce_speed"},
        {"Règle 1": "Température haute (750-800°C)", "Règle 2": "Pression haute (8.5-9.0 bar)", "Conflit": "OUI", "Résolution": "Action la plus sûre", "Action": "maintain_speed"},
    ])
    st.dataframe(conflict_df, use_container_width=True, hide_index=True)
    
    # Résumé
    st.markdown("---")
    st.markdown("### Résumé des tests de robustesse")
    
    st.success("""
    ✅ **TOUS LES TESTS SONT PASSÉS**
    
    | Scénario | Comportement attendu | Comportement observé | Statut |
    |----------|---------------------|---------------------|--------|
    | Capteur NaN | Détection + correction | ✅ Détecté et corrigé | PASSÉ |
    | Valeur hors échelle | Clip à [0,1] | ✅ Clip appliqué | PASSÉ |
    | Température > 1200°C | Valeur par défaut | ✅ Correction appliquée | PASSÉ |
    | Montée rapide (>800°C) | Interdiction increase | ✅ Interdit | PASSÉ |
    | Montée rapide (>850°C) | STOP forcé | ✅ STOP forcé | PASSÉ |
    | Conflit de règles | Résolution par priorité | ✅ Résolu | PASSÉ |
    """)
