...
class MAPPOAgentNS:
    ...
    def __init__(self, 
        ...
        if use_shield:
            ...
            self.shield = SymbolicShield()
            self.rule_priorities = {'R1': 1, 'R2': 2, 'R3': 1, 'R4': 2, 'R5': 1, 'R6': 3, 'R7': 1}
            self.shield_explanations = []
            self.stats = {'safe_actions': 0, 'corrected_actions': 0, 'rule_triggers': {}}
        else:
            self.shield = None
    ...
    def select_action(self, observation: np.ndarray, explore: bool = True) -> Tuple[int, torch.Tensor, torch.Tensor]:
        """
        Sélection d'action avec filtrage neurosymbolique
        """
        ...
        # Initialisation de final_action
        final_action = None
        ...
        if self.use_shield and self.shield:
            # Dénormalisation pour les règles
            ...
            shield_action, was_modified, explanation = self.shield.filter_action(raw_action_int, observation)
            ...
            if was_modified:
                ...
                self.shield_explanations.append({
                    'timestamp': datetime.now().isoformat(),
                    'raw_action': raw_action_int,
                ...
                # Recalculer le log_prob pour l'action modifiée
                if explore:
            ...
            # Pas de shield, action directe
            final_action = raw_action_int
    
        # Sécurité : si final_action est toujours None (cas improbable)
        if final_action is None:
            final_action = 1  # Action par défaut : MAINTAIN_SPEED
    
        return final_action, log_prob.detach(), value.detach()

    def _check_all_rules(self, action: int, obs: np.ndarray) -> List[str]:
        """Vérifie toutes les règles et retourne celles déclenchées"""
        triggered = []
        ...
        if temp > 800 and action == 2:
            triggered.append('R2')
    
        # R3: Pression critique
        if pressure >= 10 and action != 4:
            triggered.append('R3')
    
        # R4: Pression élevée
        if pressure > 9.0 and action == 2:
            triggered.append('R4')
    
        # R5: Maintenance requise
        if maint and action != 4:
            triggered.append('R5')
    
        # R6: Arrêt inutile
        if speed == 0 and temp < 800 and action in [3, 4]:
            triggered.append('R6')
    
        # R7: Conflit R1 et R3
        if temp >= 850 and pressure >= 10:
            triggered.append('R7')
    
        return triggered

    def _apply_rule(self, rule_id: str, obs: np.ndarray) -> Tuple[int, str]:
        """Applique une règle et retourne l'action sécurisée avec explication"""
        ...
        rules = {
            'R1': (4, f"Température {temp:.0f}°C ≥ 850°C → STOP"),
            'R2': (1, f"Température {temp:.0f}°C > 800°C → augmentation bloquée"),
            'R3': (4, f"Pression {pressure:.1f} bar ≥ 10 bar → STOP"),
            'R4': (1, f"Pression {pressure:.1f} bar > 9.0 bar → augmentation bloquée"),
            'R5': (4, "Maintenance requise → STOP"),
            'R6': (2, f"Arrêt inutile (v={speed:.1f}, T={temp:.0f}°C) → reprise production"),
            'R7': (4, "Conflit T°C/Pression → Sécurité machine prioritaire → STOP")
        }
    
        action, explanation = rules.get(rule_id, (1, "Règle par défaut"))
        self.stats['rule_triggers'][rule_id] = self.stats['rule_triggers'].get(rule_id, 0) + 1
        self.stats['corrected_actions'] += 1
    ...
    def load(self, path: str):
        """Charge le modèle - Version compatible PyTorch 2.6+"""
        # Pour PyTorch 2.6+, il faut mettre weights_only=False
        # car le fichier contient des objets numpy
